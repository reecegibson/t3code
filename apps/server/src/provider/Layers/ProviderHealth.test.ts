...
it.effect("returns unavailable when claude code is below the minimum supported version", () =>
  Effect.gen(function* () {
    const status = yield* checkClaudeCodeProviderStatus;
    assert.strictEqual(status.provider, "claude-code");
    assert.strictEqual(status.status, "error");
    ...
    assert.strictEqual(
      status.message,
      "Claude Code CLI v0.9.0 is too old for T3 Code. Upgrade to v1.0.0 or newer and restart T3 Code.",
    );
  }).pipe(
    Effect.provide(
      mockSpawnerLayer((args) => {
        const joined = args.join(" ");
        if (joined === "--version") return { stdout: "claude 0.9.0\n", stderr: "", code: 0 };
        throw new Error(`Unexpected args: ${joined}`);
      }),
    ),
  ),
...
it.effect("returns unauthenticated when claude code auth probe reports not authenticated", () =>
  Effect.gen(function* () {
    const status = yield* checkClaudeCodeProviderStatus;
    assert.strictEqual(status.provider, "claude-code");
    assert.strictEqual(status.status, "error");
...
it.effect(
  "returns unauthenticated when claude code auth output includes 'not logged in'",
  () =>
    Effect.gen(function* () {
      const status = yield* checkClaudeCodeProviderStatus;
      assert.strictEqual(status.provider, "claude-code");
...
it.effect("returns unauthenticated when claude code auth exits non-zero with no auth markers", () =>
  Effect.gen(function* () {
    const status = yield* checkClaudeCodeProviderStatus;
    assert.strictEqual(status.provider, "claude-code");
    assert.strictEqual(status.status, "error");
    ...
    Effect.provide(
      mockSpawnerLayer((args) => {
        ...
        if (joined === "--version") return { stdout: "claude 1.0.0\n", stderr: "", code: 0 };
        if (joined === "auth status") {
          return { stdout: "", stderr: "something unexpected", code: 2 };
        }
        throw new Error(`Unexpected args: ${joined}`);
      }),
    ),
