...
const FAKE_CODEX_NODE_SCRIPT = `\
const fs = require("node:fs");
let outputPath = "";
let seenImage = false;
const args = process.argv.slice(2);
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--image") { i++; if (i < args.length) seenImage = true; continue; }
  if (args[i] === "--output-last-message") { i++; outputPath = args[i] || ""; }
}
const chunks = [];
...
process.stdin.on("end", () => {
  ...
  if (outputPath) {
    const b64 = process.env.T3_FAKE_CODEX_OUTPUT_B64 || "e30=";
    fs.writeFileSync(outputPath, Buffer.from(b64, "base64"));
  }
  process.exit(parseInt(process.env.T3_FAKE_CODEX_EXIT_CODE || "0", 10));
});
`;
...
function makeFakeCodexBinary(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(scriptPath, FAKE_CODEX_NODE_SCRIPT);
